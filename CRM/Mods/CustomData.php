  ...
  */
  public function syncCustomGroup($source_file) {
    ...
    if (isset($data['extends_entity_column_value'])) {
      ...
      elseif ($data['extends'] == 'Contribution') {
        $extends_list = array();
        foreach ($data['extends_entity_column_value'] as $financial_type) {
          if (!is_numeric($financial_type)) {
            $financial_type = civicrm_api3('FinancialType', 'getsingle', array(
              'name' => $financial_type,
            ));
            $financial_type = $financial_type['id'];
          }
          if ($financial_type) {
            $extends_list[] = $financial_type;
          }
        }
        $data['extends_entity_column_value'] = $extends_list;
      }
